...
const EditKategoriBuku = () => {
    const {id} = useParams();
    const [kategori, setKategori] = useState("");
    const [jenis, setJenis] = useState("");
    const [titleJenis, setTitleJenis] = useState("");
...
const submitData = (e) => {
  e.preventDefault();
  axios
    .post(`${url}/v1/api/edit-kategori/${id}`, {
      title: kategori,
      idJenis: jenis,
    })
    .then((res) => {
...
const getApiViewJenis = () => {
    axios.get(`${url}/v1/api/jenis-kategori`).then(
        result=> {
            console.log(result);
...
const getDetailKategori=()=>{
    axios
    .get(`${url}/v1/api/detail-kategori/${id}`)
    .then((res) => {
      console.log(res);
      setJenis(res.data.kategori.idJenis._id)
      setIdJenis(res.data.kategori.idJenis._id)
      setTitleJenis(res.data.kategori.idJenis.title)
      setKategori(res.data.kategori.title)
    })
    .catch((err) => {
      console.log(err);
    });
    ...
    <div className="flex">
      ...
      <div className="w-full px-6 py-4 my-auto mx-auto overflow-hidden bg-white shadow-md sm:max-w-lg sm:rounded-lg">
          ...
          <div className="relative z-0 mb-6 w-full group">
            ...
            >
              <option value={`${idJenis}`}>{titleJenis}</option>
              <option value="">--- Jenis Buku ---</option>
              {
                    dataJenis && dataJenis.map((item, index)=> (
          ...
          {
                    dataJenis && dataJenis.map((item, index)=> {
                        <h1 key={index} value={`${item._id}`}>{item.title}</h1>
                    })
                }
          ...
          <div
            ...
            role="alert"
          >
            <span className="font-medium">Berhasil edit kategori</span>
          </div>
        ) : (
          ""
        )}
